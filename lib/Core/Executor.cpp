...
ref<Expr> Executor::concretizeReadExpr(const ExecutionState &state, ref<Expr> &expr) {
//    errs() << "\n[concretizeReadExpr]\n";
...
ref<Expr> Executor::concretizeExpr(const klee::ExecutionState &state, klee::ref<klee::Expr> &expr) {
//    errs() << "\n[concretzeExpr]\n";
...
ExprPPrinter::printQuery(llvm::errs(), state
...
++i)
res.
push_back(std::make_pair(state.symbolics[i].first->name, values[i])
);
return true;
}
